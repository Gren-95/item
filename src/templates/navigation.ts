import { SEARCH_ICON, LOCATION_ICON, COG_ICON, SHOPPING_BAG_ICON, X_CIRCLE_ICON, WRENCH_ICON, USER_GROUP_ICON, LOCK_CLOSED_ICON, CLIPBOARD_CHECK_ICON, LOGOUT_ICON, PRINTER_ICON, SUN_ICON, MOON_ICON, COMPUTER_DESKTOP_ICON, PLUS_ICON, MINUS_ICON, REFRESH_ICON, CHEVRON_DOWN_ICON } from "./icons";
...
const MGMT_LINK_CLASSES = "flex items-center gap-3 px-3 py-2 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-200/70 dark:hover:bg-gray-600 font-medium transition-all";
    ...
    <div id="nav-links" class="hidden flex-col absolute left-0 top-full mt-2 bg-white dark:bg-gray-800 shadow-xl rounded-xl border border-gray-200 dark:border-gray-700 min-w-[260px] z-50 transition-all overflow-hidden">
      ...
      <div class="p-2">
        ...
        <div id="management-section">
          <button id="management-toggle" class="flex items-center justify-between w-full px-3 py-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 font-semibold transition-all text-sm" aria-expanded="false">
            <span>Management</span>
            <span id="management-chevron" class="transition-transform duration-200">
              ${CHEVRON_DOWN_ICON.replace('w-5 h-5', 'w-4 h-4')}
            </span>
          </button>
          <div id="management-links" class="hidden">
            <nav class="mt-1 ml-2 space-y-1 pl-2 border-l-2 border-gray-200 dark:border-gray-600">
              <a href="/locations" class="${MGMT_LINK_CLASSES}">
                ${menuIcon(LOCATION_ICON)}
                Locations
              </a>
              <a href="/types" class="${MGMT_LINK_CLASSES}">
                ${menuIcon(COG_ICON)}
                Configurations
              </a>
              <a href="/vendors" class="${MGMT_LINK_CLASSES}">
                ${menuIcon(SHOPPING_BAG_ICON)}
                Providers
              </a>
              <a href="/write-off-reasons" class="${MGMT_LINK_CLASSES}">
                ${menuIcon(X_CIRCLE_ICON)}
                Write-Off Reasons
              </a>
              ${isAdmin
                ? `<a href="/permissions" class="${MGMT_LINK_CLASSES}">${menuIcon(USER_GROUP_ICON)}User Permissions</a>`
                : `<span class="${MENU_LINK_DISABLED} ml-2 pl-2">${menuIcon(USER_GROUP_ICON)}User Permissions</span>`
              }
            </nav>
          </div>
        </div>
